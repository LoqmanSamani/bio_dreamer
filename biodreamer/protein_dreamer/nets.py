...
class DiffusionPredictor(nn.Module):
    ...
    def __init__(self, predictor: Any, diffusion_type: Optional[str] = None, pred_type: Optional[str] = None, schedule_type: Optional[str] = None, device: Optional[torch.device] = None) -> None:
        super().__init__()
        ...
        self.diffusion_type = diffusion_type if diffusion_type is not None else 'ddpm'  # default to simple diffusion if not specified
    ...
    def _setup_schedule_ddpm(self) -> None:
    ...
    def predict_x0_ddpm(self, xt: torch.Tensor, t: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
    ...
    def get_variance_ddpm(self, t: torch.Tensor, pred_var: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def reverse_ddpm(
        ...
        pred_x0 = self.predict_x0_ddpm(xt, t, pred)
        ...
        if self.var_type == "fixed_small":
            ...
            variance = self.get_variance_ddpm(t, pred_var)
